...
app.get("/", function(req, res) {
	Article.find({}, null, {sort: {created: -1}}, function(err, data) {
		if(data.length === 0) {
			res.render("placeholder", {message: "Please click \"Scrape For Newest Articles\" for latest the news."});
		}
		else{
...
app.get("/saved", function(req, res) {
	Article.find({issaved: true}, null, {sort: {created: -1}}, function(err, data) {
		if(data.length === 0) {
			res.render("placeholder", {message: "No Saved Articles. Click \"Save Article\" to save your favorites!"});
		}
		else {
